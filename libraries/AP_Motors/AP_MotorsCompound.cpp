...
/*
 *       AP_MotorsCompound.cpp - ArduCopter motors library
 *       Code by RandyMackay. DIYDrones.com
 *
 */
#include <stdlib.h>
#include <AP_HAL.h>
#include "AP_MotorsCompound.h"

extern const AP_HAL::HAL& hal;

const AP_Param::GroupInfo AP_MotorsCompound::var_info[] PROGMEM = {
    ...
    // @User: Standard
    // @Increment: 1
    AP_GROUPINFO("SV1_POS", 1,      AP_MotorsCompound,  _servo1_pos, AP_MOTORS_COMPOUND_SERVO1_POS),
    ...
    // @User: Standard
    // @Increment: 1
    AP_GROUPINFO("SV2_POS", 2,      AP_MotorsCompound,  _servo2_pos,  AP_MOTORS_COMPOUND_SERVO2_POS),
    ...
    // @User: Standard
    // @Increment: 1
    AP_GROUPINFO("SV3_POS", 3,      AP_MotorsCompound,  _servo3_pos,  AP_MOTORS_COMPOUND_SERVO3_POS),
    ...
    // @Increment: 100
    // @User: Advanced
    AP_GROUPINFO("ROL_MAX", 4,      AP_MotorsCompound,  _roll_max,    AP_MOTORS_COMPOUND_SWASH_ROLL_MAX),
    ...
    // @Increment: 100
    // @User: Advanced
    AP_GROUPINFO("PIT_MAX", 5,      AP_MotorsCompound,  _pitch_max,   AP_MOTORS_COMPOUND_SWASH_PITCH_MAX),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("COL_MIN", 6,      AP_MotorsCompound,  _collective_min, AP_MOTORS_COMPOUND_COLLECTIVE_MIN),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("COL_MAX", 7,      AP_MotorsCompound,  _collective_max, AP_MOTORS_COMPOUND_COLLECTIVE_MAX),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("COL_MID", 8,      AP_MotorsCompound,  _collective_mid, AP_MOTORS_COMPOUND_COLLECTIVE_MID),
    ...
    // @User: Standard
    AP_GROUPINFO("TAIL_TYPE",9,     AP_MotorsCompound,  _tail_type, AP_MOTORS_COMPOUND_TAILTYPE_SERVO),
    ...
    // @User: Standard
    AP_GROUPINFO("SWASH_TYPE",10,   AP_MotorsCompound,  _swash_type, AP_MOTORS_COMPOUND_SWASH_CCPM),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("GYR_GAIN",    11,     AP_MotorsCompound,  _ext_gyro_gain, AP_MOTORS_COMPOUND_EXT_GYRO_GAIN),
    ...
    // @User: Standard
    AP_GROUPINFO("SV_MAN",  12,     AP_MotorsCompound,  _servo_manual,  0),
    ...
    // @User: Advanced
    // @Increment: 1
    AP_GROUPINFO("PHANG",   13,     AP_MotorsCompound,  _phase_angle,   0),
    ...
    // @Range: -10 10
    AP_GROUPINFO("COLYAW",  14,     AP_MotorsCompound,  _collective_yaw_effect, 0),
    ...
    // @Increment: 10
    // @User: Standard
    AP_GROUPINFO("RSC_SETPOINT", 15, AP_MotorsCompound, _rsc_setpoint, AP_MOTORS_COMPOUND_RSC_SETPOINT),
    ...
    // @User: Standard
    AP_GROUPINFO("RSC_MODE", 16, AP_MotorsCompound,     _rsc_mode, AP_MOTORS_COMPOUND_RSC_MODE_CH8_PASSTHROUGH),
    ...
    // @User: Standard
    AP_GROUPINFO("FLYBAR_MODE", 18, AP_MotorsCompound,  _flybar_mode, AP_MOTORS_COMPOUND_NOFLYBAR),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("LAND_COL_MIN", 21, AP_MotorsCompound, _land_collective_min, AP_MOTORS_COMPOUND_LAND_COLLECTIVE_MIN),
    ...
    // @Units: Seconds
    // @User: Standard
    AP_GROUPINFO("RSC_RAMP_TIME", 22, AP_MotorsCompound,_rsc_ramp_time, AP_MOTORS_COMPOUND_RSC_RAMP_TIME),
    ...
    // @Units: Seconds
    // @User: Standard
    AP_GROUPINFO("RSC_RUNUP_TIME", 23, AP_MotorsCompound,_rsc_runup_time, AP_MOTORS_COMPOUND_RSC_RUNUP_TIME),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("TAIL_SPEED", 24, AP_MotorsCompound,  _direct_drive_tailspeed, AP_MOTOR_COMPOUND_DDTAIL_DEFAULT),
...
void AP_MotorsCompound::set_update_rate( uint16_t speed_hz )
{
...
void AP_MotorsCompound::output_min()
{
    // move swash to mid
...
void AP_MotorsCompound::output_test(uint8_t motor_seq, int16_t pwm)
{
    ...
    switch (motor_seq) {
        ...
        case 4:
            ...
            if (_tail_type == AP_MOTORS_COMPOUND_TAILTYPE_SERVO_EXTGYRO) {
        ...
        case 5:
            // main rotor
            hal.rcout->write(pgm_read_byte(&_motor_to_channel_map[AP_MOTORS_COMPOUND_RSC]), pwm);
            break;
        default:
            // do nothing
            break;
...
bool AP_MotorsCompound::allow_arming() const
{
    // ensure main rotor has started
    if (_rsc_mode != AP_MOTORS_COMPOUND_RSC_MODE_NONE && _servo_rsc.control_in > 0) {
        return false;
    }
...
void AP_MotorsCompound::set_desired_rotor_speed(int16_t desired_speed)
{
    _rotor_desired = desired_speed;
}

// return true if the main rotor is up to speed
bool AP_MotorsCompound::motor_runup_complete() const
{
    // if we have no control of motors, assume pilot has spun them up
    if (_rsc_mode == AP_MOTORS_COMPOUND_RSC_MODE_NONE) {
        return true;
    }
...
void AP_MotorsCompound::recalc_scalers()
{
...
void AP_MotorsCompound::output_armed()
{
...
void AP_MotorsCompound::output_disarmed()
{
...
void AP_MotorsCompound::reset_swash()
{
...
void AP_MotorsCompound::init_swash()
{
...
void AP_MotorsCompound::calculate_roll_pitch_collective_factors()
{
    if (_swash_type == AP_MOTORS_COMPOUND_SWASH_CCPM) {                     //CCPM Swashplate, perform control mixing
...
//                       yaw:   -4500 ~ 4500
//
void AP_MotorsCompound::move_swash(int16_t roll_out, int16_t pitch_out, int16_t coll_in, int16_t yaw_out)
{
    ...
    if (_servo_manual == 1) {      // are we in manual servo mode? (i.e. swash set-up mode)?
        ...
        if ((_rotor_desired > 0) && _tail_type != AP_MOTORS_COMPOUND_TAILTYPE_SERVO_EXTGYRO) {
    ...
    if (_swash_type == AP_MOTORS_COMPOUND_SWASH_H1) {
...
void AP_MotorsCompound::rsc_control()
{
    // if disarmed output minimums
    if (!armed()) {
        // shut down tail rotor
        if (_tail_type == AP_MOTORS_COMPOUND_TAILTYPE_DIRECTDRIVE_VARPITCH || _tail_type == AP_MOTORS_COMPOUND_TAILTYPE_DIRECTDRIVE_FIXEDPITCH) {
    ...
    if (_tail_type == AP_MOTORS_COMPOUND_TAILTYPE_DIRECTDRIVE_FIXEDPITCH) {
...
void AP_MotorsCompound::rotor_ramp(int16_t rotor_target)
{
...
void AP_MotorsCompound::tail_ramp(int16_t tail_target)
{
    // return immediately if not ramping required
    if (_tail_type != AP_MOTORS_COMPOUND_TAILTYPE_DIRECTDRIVE_VARPITCH) {
    ...
    if (_tail_direct_drive_out < tail_target) {
        _tail_direct_drive_out += AP_MOTORS_COMPOUND_TAIL_RAMP_INCREMENT;
        if (_tail_direct_drive_out >= tail_target) {
            _tail_direct_drive_out = tail_target;
        }
...
bool AP_MotorsCompound::tail_rotor_runup_complete()
{
...
void AP_MotorsCompound::write_rsc(int16_t servo_out)
{
    _servo_rsc.servo_out = servo_out;
    _servo_rsc.calc_pwm();
    hal.rcout->write(AP_MOTORS_COMPOUND_RSC, _servo_rsc.radio_out);
}

// write_aux - outputs pwm onto output aux channel (ch7)
// servo_out parameter is of the range 0 ~ 1000
void AP_MotorsCompound::write_aux(int16_t servo_out)
{
    _servo_aux.servo_out = servo_out;
    _servo_aux.calc_pwm();
    hal.rcout->write(AP_MOTORS_COMPOUND_AUX, _servo_aux.radio_out);
}

// set_delta_phase_angle for setting variable phase angle compensation and force
// recalculation of collective factors
void AP_MotorsCompound::set_delta_phase_angle(int16_t angle)
{
