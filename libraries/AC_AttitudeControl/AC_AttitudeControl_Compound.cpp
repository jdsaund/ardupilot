...
const AP_Param::GroupInfo AC_AttitudeControl_Compound::var_info[] PROGMEM = {
    ...
    // @Increment: 500
    // @User: Advanced
    AP_GROUPINFO("RATE_RP_MAX", 0, AC_AttitudeControl_Compound, _angle_rate_rp_max, AC_ATTITUDE_CONTROL_RATE_RP_MAX_DEFAULT),
    ...
    // @Increment: 500
    // @User: Advanced
    AP_GROUPINFO("RATE_Y_MAX",  1, AC_AttitudeControl_Compound, _angle_rate_y_max, AC_ATTITUDE_CONTROL_RATE_Y_MAX_DEFAULT),
    ...
    // @Increment: 100
    // @User: Advanced
    AP_GROUPINFO("SLEW_YAW",    2, AC_AttitudeControl_Compound, _slew_yaw, AC_ATTITUDE_CONTROL_SLEW_YAW_DEFAULT),
    ...
    // @Increment: 1000
    // @User: Advanced
    AP_GROUPINFO("ACCEL_Y_MAX",  4, AC_AttitudeControl_Compound, _accel_yaw_max, AC_ATTITUDE_CONTROL_ACCEL_Y_MAX_DEFAULT),
    ...
    // @User: Advanced
    AP_GROUPINFO("RATE_FF_ENAB", 5, AC_AttitudeControl_Compound, _rate_bf_ff_enabled, AC_ATTITUDE_CONTROL_RATE_BF_FF_DEFAULT),
    ...
    // @User: Advanced
    AP_GROUPINFO("ACCEL_R_MAX", 6, AC_AttitudeControl_Compound, _accel_roll_max, AC_ATTITUDE_CONTROL_ACCEL_RP_MAX_DEFAULT),
    ...
    // @Values: 0:Disabled, 72000:Slow, 108000:Medium, 162000:Fast
    // @User: Advanced
    AP_GROUPINFO("ACCEL_P_MAX", 7, AC_AttitudeControl_Compound, _accel_pitch_max, AC_ATTITUDE_CONTROL_ACCEL_RP_MAX_DEFAULT),
...
void AC_AttitudeControl_Compound::passthrough_bf_roll_pitch_rate_yaw(float roll_passthrough, float pitch_passthrough, float yaw_rate_bf)
{
    ...
    _flags_compound.flybar_passthrough = true;
...
void AC_AttitudeControl_Compound::rate_controller_run()
{	
    // call rate controllers and send output to motors object
    // if using a flybar passthrough roll and pitch directly to motors
    if (_flags_compound.flybar_passthrough) {
...
void AC_AttitudeControl_Compound::rate_bf_to_motor_roll_pitch(float rate_roll_target_cds, float rate_pitch_target_cds)
{
    ...
    if (!_flags_compound.limit_roll || ((roll_i>0&&rate_roll_error<0)||(roll_i<0&&rate_roll_error>0))){
        if (((AP_MotorsCompound&)_motors).has_flybar()) {                              // Mechanical Flybars get regular integral for rate auto trim
            if (rate_roll_target_cds > -50 && rate_roll_target_cds < 50){       // Frozen at high rates
                roll_i = _pid_rate_roll.get_i();
            }
        }else{
            if (_flags_compound.leaky_i){
                roll_i = ((AC_HELI_PID&)_pid_rate_roll).get_leaky_i(AC_ATTITUDE_COMPOUND_RATE_INTEGRATOR_LEAK_RATE);
            }else{
                roll_i = _pid_rate_roll.get_i();
            }
        }
    ...
    // update i term as long as we haven't breached the limits or the I term will certainly reduce
    if (!_flags_compound.limit_pitch || ((pitch_i>0&&rate_pitch_error<0)||(pitch_i<0&&rate_pitch_error>0))){
        if (((AP_MotorsCompound&)_motors).has_flybar()) {                              // Mechanical Flybars get regular integral for rate auto trim
            ...
            if (_flags_compound.leaky_i) {
                pitch_i = ((AC_HELI_PID&)_pid_rate_pitch).get_leaky_i(AC_ATTITUDE_COMPOUND_RATE_INTEGRATOR_LEAK_RATE);
            }else{
                pitch_i = _pid_rate_pitch.get_i();
            }
        }
    ...
    if ((float)fabs(roll_out) > AC_ATTITUDE_RATE_RP_CONTROLLER_OUT_MAX) {
        ...
        _flags_compound.limit_roll = true;
    }else{
        _flags_compound.limit_roll = false;
    }
    if ((float)fabs(pitch_out) > AC_ATTITUDE_RATE_RP_CONTROLLER_OUT_MAX) {
        ...
        _flags_compound.limit_pitch = true;
    }else{
        _flags_compound.limit_pitch = false;
    }
...
float AC_AttitudeControl_Compound::rate_bf_to_motor_yaw(float rate_target_cds)
{
    ...
    // update i term as long as we haven't breached the limits or the I term will certainly reduce
    if (!_flags_compound.limit_yaw || ((i>0&&rate_error<0)||(i<0&&rate_error>0))) {
        if (((AP_MotorsCompound&)_motors).motor_runup_complete()) {
            i = _pid_rate_yaw.get_i();
        } else {
            i = ((AC_HELI_PID&)_pid_rate_yaw).get_leaky_i(AC_ATTITUDE_COMPOUND_RATE_INTEGRATOR_LEAK_RATE);    // If motor is not running use leaky I-term to avoid excessive build-up
        }
    }
    ...
    if ((float)fabs(yaw_out) > AC_ATTITUDE_RATE_YAW_CONTROLLER_OUT_MAX) {
        ...
        _flags_compound.limit_yaw = true;
    }else{
        _flags_compound.limit_yaw = false;
    }
...
int16_t AC_AttitudeControl_Compound::get_angle_boost(int16_t throttle_pwm)
{
...
void AC_AttitudeControl_Compound::update_feedforward_filter_rates(float time_step)
{
    pitch_feedforward_filter.set_cutoff_frequency(time_step, AC_ATTITUDE_COMPOUND_RATE_FF_FILTER);
    roll_feedforward_filter.set_cutoff_frequency(time_step, AC_ATTITUDE_COMPOUND_RATE_FF_FILTER);
    yaw_feedforward_filter.set_cutoff_frequency(time_step, AC_ATTITUDE_COMPOUND_RATE_FF_FILTER);
}
