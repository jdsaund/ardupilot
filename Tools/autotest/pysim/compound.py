...
class Compound(Aircraft):
    '''a Compound HeliCopter simulator'''
    ...
    def update(self, servos):
        ...
        thrust = (rsc/self.rsc_setpoint)*(swash1+swash2+swash3-1.5)*2.0 + (self.velocity_body.z * 0.2)

        # very simplistic mapping to body euler rates
        roll_rate = (swash1 - swash2) * 10
        pitch_rate = ((swash1 + swash2)/2.0 - swash3) * 10
	...
	# fixed wing aerodynamics - very primitive
        thrust *= self.thrust_scale
	prop_thrust = ((fwd_thrust * 20.0) - self.velocity_body.x)
	(rol, pit, yaw) = self.dcm.to_euler()
	vel_x_squared = self.velocity_body.x * self.velocity_body.x
	if self.velocity_body.x == 0:
		trajectory_angle = 0
	else:
		trajectory_angle = degrees(atan (-self.velocity_body.z / self.velocity_body.x))
	...
	self.velocity_body = self.dcm.transposed() * self.velocity
